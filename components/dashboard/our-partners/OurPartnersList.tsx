...
import { OurPartnersFormModal } from '@/components/dashboard/our-partners/OurPartnersFormModal';
import {
	deleteOurPartner,
	fetchOurPartners,
} from '@/lib/our-partner-api';
import { getOurPartnerLogoUrl, isUsingFrontendPartnerLogo, sortOurPartners } from '@/lib/our-partner-utils';
import type { OurPartner } from '@/types/our-partner';

export function OurPartnersList() {
	const [partners, setPartners] = useState<OurPartner[]>([]);
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState('');
	const [modalPartner, setModalPartner] = useState<OurPartner | null | undefined>(
		undefined,
	);
	...
	const loadPartners = useCallback(async () => {
		...
		try {
			...
			setPartners(sortOurPartners(data));
		} catch (loadError) {
			setError(
	...
	const handleDelete = async (partner: OurPartner) => {
		...
		try {
			await deleteOurPartner(partner.id);
			setPartners((current) =>
	...
	const handleSaved = (saved: OurPartner) => {
		setPartners((current) => {
			const existingIndex = current.findIndex((item) => item.id === saved.id);

			if (existingIndex === -1) {
				return sortOurPartners([...current, saved]);
			}

			const next = [...current];
			next[existingIndex] = saved;
			return sortOurPartners(next);
		});
	};
	...
	return (
		<div className='space-y-6'>
			<div className='flex flex-wrap items-center justify-between gap-3'>
				<div>
					<h1 className='text-2xl font-bold text-slate-900'>Our partners</h1>
					<p className='mt-1 text-sm text-slate-600'>
						Manage property management software partner logos shown in the
						&quot;Our Property Management Software Partners&quot; section.
					</p>
					<p className='mt-1 text-xs text-slate-500'>
						This is separate from Partners network (Home Depot, NARPM, etc.).
					</p>
				</div>
				<button
					type='button'
			...
			{!loading && !error && partners.length === 0 ? (
				<div className='rounded-xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center'>
					<p className='text-sm text-slate-600'>
						No software partners yet. Add your first partner logo.
					</p>
				</div>
			) : null}
